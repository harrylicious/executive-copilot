...
class CustomRetriever(BaseRetriever):
    ...
    Attributes:
        ...
        min_score: Min similarity score for local/combined (default 0.3, range 0.0-1.0).
        ...
        language: Query language code (default "id"). Indonesian queries use a
            lower min_score threshold since product/item names in Bahasa Indonesia
            tend to score lower with multilingual embedding models.
    """
    ...
    # score lower due to mixing Bahasa Indonesia with product codes/names
    _INDONESIAN_MIN_SCORE: float = 0.01

    class Config:
        arbitrary_types_allowed = True

    def _effective_min_score(self) -> float:
        """Return the effective min_score based on the configured language.

        Indonesian queries use a lower threshold because product names and
        tabular data in Bahasa Indonesia score lower with the all-MiniLM-L6-v2
        embedding model.

        Returns:
            The min_score to use for filtering results.
        """
        if self.language == "id":
    ...
    def _execute_search(self, query: str) -> SearchResult:
        ...
        Uses _effective_min_score() to determine the actual threshold,
        which may be lower for Indonesian language queries.
        ...
        Raises:
            ValueError: Propagated from RetrievalService for invalid queries.
        """
        effective_score = self._effective_min_score()
