...
from app.services.langchain.rag_chain import RAGChain, RAGResponse, _ERROR_RESPONSE
...
class TestRAGChainInvoke:
    ...
    def test_invoke_zero_documents_still_calls_llm_with_empty_context(self):
        ...
        # LLM is called with empty context — prompt's built-in rule handles the response
        assert result.answer == "Data tidak ditemukan dalam dokumen yang tersedia"
...
class TestRAGChainAsync:
    ...
    @pytest.mark.asyncio
    async def test_ainvoke_zero_documents_still_calls_llm(self):
        mock_retriever = _make_mock_retriever([])
        mock_llm = _make_mock_llm("Data tidak ditemukan dalam dokumen yang tersedia")
    ...
    @pytest.mark.asyncio
    async def test_astream_zero_documents_still_streams_from_llm(self):
        mock_retriever = _make_mock_retriever([])

        # Mock streaming LLM
        mock_llm = MagicMock()
        chunks = [
            MagicMock(content="Data tidak ditemukan"),
            MagicMock(content=" dalam dokumen yang tersedia"),
        ]
        ...
        assert tokens == ["Data tidak ditemukan", " dalam dokumen yang tersedia"]
