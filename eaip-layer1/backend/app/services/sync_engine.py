...
class SyncEngine:
    ...
    def _remove_deleted_files(
        ...
        for path in deleted_paths:
            ...
            try:
                self._cleanup_graphrag_data(db_file.id)
            except Exception:
    ...
    def _cleanup_graphrag_data(self, file_id: int) -> None:
        from app.config import graphrag_settings

        chunk_ids = [
            c[0]
            for c in self.db.query(Chunk.id)
        ...
        if chunk_ids:
            ...
            for entity in all_entities:
                entity_chunk_ids = entity.source_chunk_ids or []
                if any(cid in chunk_ids for cid in entity_chunk_ids):
