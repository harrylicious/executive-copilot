...
from app.config import graphrag_settings, ingestion_settings
...
class IngestionOrchestrator:
    ...
    def _run_embedding(
        ...
        """Run the embedding stage: generate embeddings and store in ChromaDB + SQLite.

        Creates a File record, generates embeddings for each chunk, stores them
        in ChromaDB via the vector store, and persists Chunk records in SQLite.
        ...
        try:
            ...
            embedding_model = EmbeddingModel(graphrag_settings.embedding_model)
            chunk_texts = [c.text for c in chunks]
            embeddings = embedding_model.embed_texts(chunk_texts)

            # Step 3: Store in ChromaDB via vector store
            vector_store = ChromaVectorStore(graphrag_settings)

            # Adapt SemanticChunkResult to the ChunkResult interface expected by vector store
            from app.services.document_chunker import ChunkResult as DocChunkResult
            ...
            self._complete_stage_log(stage_log, "completed", details={
                ...
                "file_id": file_record.id,
                "embedding_model": graphrag_settings.embedding_model,
            })
