"""EmbeddingEngine orchestrator for the GraphRAG embedding pipeline.

Coordinates document chunking, embedding generation, vector storage, SQLite
chunk persistence, and graph extraction for files in the knowledge base.
...
class EmbeddingEngine:
    ...
    Manages the full embedding pipeline: text extraction, chunking, embedding
    generation, vector store persistence, SQLite chunk storage, and graph
    ...
    def __init__(self, db: Session, config: GraphRAGSettings, kb_path: str = "./knowledge_base"):
        self.db = db
        self.config = config
    ...
    def run_full(self) -> EmbeddingJobResult:
        ...
        ignoring content_hash comparison.
    ...
    def _process_file(self, file: File) -> tuple[int, list[dict]]:
        ...
        6. Trigger graph entity/relationship extraction
        ...
        # Step 4: Store embeddings in ChromaDB vector store
        try:
            metadata = {"department": file.department or ""}
            ...
            logger.error(
                f"Vector store upsert failed for file_id={file.id}: {e}"
            )
            errors.append(
                {
                    "file_id": file.id,
        ...
        # Step 6: Trigger graph entity/relationship extraction
        try:
            self.graphrag.extract_entities_and_relationships(db_chunks, file.id)
        except Exception as e:
            logger.error(
                f"Graph extraction failed for file_id={file.id}: {e}"
            )
            # Graph extraction failure is non-fatal for the embedding pipeline
            errors.append(
                {
                    "file_id": file.id,
                    "file_path": file.path,
