...
class TestCreateBatchConfig:
    ...
    def test_create_config_with_local_path(self, client):
        ...
        assert data["source_path"] == "/data/shared/documents"
    ...
    def test_create_config_with_s3_uri(self, client):
        ...
        data = response.json()
        assert data["source_path"] == "s3://my-bucket/documents/incoming"
...
class TestUpdateBatchConfig:
    ...
    def test_update_name(self, client, sample_config):
        ...
        assert data["source_path"] == sample_config.source_path
    ...
    def test_update_source_path_valid(self, client, sample_config):
        ...
        assert response.status_code == 200
        assert response.json()["source_path"] == "/new/valid/path"
...
class TestListBatchConfigExecutions:
    ...
    def test_list_executions_with_data(self, client, sample_config, db_session):
        ...
        data = response.json()
        assert len(data) == 1
        assert data[0]["config_id"] == sample_config.id
        assert data[0]["files_found"] == 10
        assert data[0]["files_submitted"] == 8
        assert data[0]["files_skipped"] == 2
